...
from pipeline import (
    load_xfoil_encoder,
    load_latent_mapper,
    load_airfoil_decoder,
    LATENT_DIM,
    INPUT_DIM_XFOIL,
    ALPHA_LENGTH,
    TARGET_POINTS,
    DEVICE as PIPELINE_DEVICE
)
# Correction model not used in new pipeline
CORRECTION_MODEL_AVAILABLE = False

# Utility functions
def compute_reynolds_mach(chord_ft, speed_mph):
    """
    Compute Reynolds number and Mach number from chord length and speed.
    ...
    """
    # Constants
    RHO = 0.002377  # Air density at sea level (slugs/ft^3)
    MU = 3.737e-7  # Dynamic viscosity at sea level (slug/(ft*s))
    A = 1116.4  # Speed of sound at sea level (ft/s)
    
    # Convert speed from mph to ft/s
    speed_fts = speed_mph * 5280 / 3600
    
    # Compute Reynolds number: Re = (rho * V * c) / mu
    Re = (RHO * speed_fts * chord_ft) / MU
...
def bin_reynolds_number(re):
    """Bin Reynolds number to nearest standard bin."""
    bins = [50000, 100000, 250000, 500000, 750000, 1000000]
    return min(bins, key=lambda x: abs(x - re))

def get_max_cl_ld_for_re_bin(re_bin):
    """
    Get maximum Cl and L/D values for scaling charts based on Re bin.
    These are approximate maximums for typical airfoils at each Re range.
    """
    max_values = {
        50000: (1.5, 80),
        100000: (1.6, 90),
    ...
    return max_values.get(re_bin, (2.0, 130))
...
def compute_cd_from_cl_clcd(cl_vector, cl_cd_vector):
    ...
    cl_arr = np.array(cl_vector)
    cl_cd_arr = np.array(cl_cd_vector)
    
    # Avoid division by zero
    cd_arr = np.where(cl_cd_arr != 0, cl_arr / cl_cd_arr, 0.0)
    
    return cd_arr

def compute_summary_statistics(alpha, cl, cd, cl_cd):
    """
    Compute summary statistics from aerodynamic curves.
    Returns dict with clmax, cdmin, ldmax (cl/cd max), and their respective alpha values.
    """
...
xfoil_scaler = None
latent_mapper = None
mapper_xfoil_scaler = None
mapper_airfoil_scaler = None
airfoil_decoder = None
airfoil_scaler = None
device = None
...
def convert_to_xfoil_format(re, mach, alpha, cl, cd, cl_cd):
    """
    Convert user input format to xfoil pipeline format.
    XFoil format: [reynolds_log, alpha[0..24], cl[0..24], cd[0..24], l_d[0..24]]
    """
    # Normalize Reynolds (log scale)
    reynolds_log = np.log10(re)
    
    # Interpolate/resample to ALPHA_LENGTH points if needed
    alpha_arr = np.array(alpha)
    cl_arr = np.array(cl)
    cd_arr = np.array(cd)
    cl_cd_arr = np.array(cl_cd)
    
    if len(alpha_arr) != ALPHA_LENGTH:
        # Resample to ALPHA_LENGTH points
        alpha_min = np.min(alpha_arr)
        alpha_max = np.max(alpha_arr)
        alpha_target = np.linspace(alpha_min, alpha_max, ALPHA_LENGTH)
        
        from scipy.interpolate import interp1d
        cl_interp = interp1d(alpha_arr, cl_arr, kind='linear', bounds_error=False, fill_value='extrapolate')
        cd_interp = interp1d(alpha_arr, cd_arr, kind='linear', bounds_error=False, fill_value='extrapolate')
        cl_cd_interp = interp1d(alpha_arr, cl_cd_arr, kind='linear', bounds_error=False, fill_value='extrapolate')
        
        alpha_resampled = alpha_target
        cl_resampled = cl_interp(alpha_target)
        cd_resampled = cd_interp(alpha_target)
        cl_cd_resampled = cl_cd_interp(alpha_target)
    else:
        alpha_resampled = alpha_arr
        cl_resampled = cl_arr
        cd_resampled = cd_arr
        cl_cd_resampled = cl_cd_arr
    
    # Flatten: [reynolds_log, alpha, cl, cd, l_d]
    flattened = np.array([reynolds_log] + alpha_resampled.tolist() + cl_resampled.tolist() + 
                         cd_resampled.tolist() + cl_cd_resampled.tolist())
    
    return flattened

def run_pipeline_new(re, mach, alpha, cl, cd, cl_cd):
    """
    Run the new pipeline: XFoil format -> encode -> latent mapper -> decode -> coordinates.
    """
    global xfoil_encoder, xfoil_scaler, latent_mapper, mapper_xfoil_scaler, mapper_airfoil_scaler
    global airfoil_decoder, airfoil_scaler, device
    
    if xfoil_encoder is None or latent_mapper is None or airfoil_decoder is None:
        raise RuntimeError("Models not loaded")
    
    # Convert to xfoil format
    performance_data = convert_to_xfoil_format(re, mach, alpha, cl, cd, cl_cd)
    ...
    x_coords = reconstructed[:TARGET_POINTS]
    y_coords = reconstructed[TARGET_POINTS:]
...
def load_models():
    ...
    # Temporarily set DEVICE and MODELS_DIR in pipeline module
    import pipeline
    pipeline.DEVICE = device
    pipeline.MODELS_DIR = Path(__file__).parent / "models"
    
    # Load models from pipeline
    xfoil_encoder, xfoil_scaler, _ = load_xfoil_encoder()
    latent_mapper, mapper_xfoil_scaler, mapper_airfoil_scaler = load_latent_mapper()
    airfoil_decoder, airfoil_scaler, _ = load_airfoil_decoder()
    
    if xfoil_encoder is None or latent_mapper is None or airfoil_decoder is None:
        raise FileNotFoundError("Could not load all required models from pipeline")
    
    # Move models to device
    xfoil_encoder = xfoil_encoder.to(device)
    latent_mapper = latent_mapper.to(device)
    airfoil_decoder = airfoil_decoder.to(device)
    
    # Apply quantization
    print("Applying dynamic quantization to models...")
    try:
        xfoil_encoder = torch.quantization.quantize_dynamic(xfoil_encoder, {nn.Linear}, dtype=torch.qint8)
        print("✓ XFoil encoder quantized")
    except Exception as e:
        print(f"⚠ XFoil encoder quantization failed: {e}")
    
    try:
        latent_mapper = torch.quantization.quantize_dynamic(latent_mapper, {nn.Linear}, dtype=torch.qint8)
        print("✓ Latent mapper quantized")
    except Exception as e:
        print(f"⚠ Latent mapper quantization failed: {e}")
    
    try:
        airfoil_decoder = torch.quantization.quantize_dynamic(airfoil_decoder, {nn.Linear}, dtype=torch.qint8)
        print("✓ Airfoil decoder quantized")
    except Exception as e:
        print(f"⚠ Airfoil decoder quantization failed: {e}")
...
def api_generate_alpha():
    ...
    # Validate angle range: -10 to 15 degrees
    if a_min < -10 or a_min > 15:
        return jsonify({'error': 'Minimum angle must be between -10 and 15 degrees'}), 400
    if a_max < -10 or a_max > 15:
        return jsonify({'error': 'Maximum angle must be between -10 and 15 degrees'}), 400
    if a_min >= a_max:
        return jsonify({'error': 'Minimum angle must be less than maximum angle'}), 400
...
def api_generate_airfoil():
    ...
    try:
        ...
        # Run pipeline (note: thickness parameters not used in new pipeline)
        pred_x, pred_y = run_pipeline_new(Re, Mach, alpha, cl, cd, cl_cd)
...
def generate_and_evaluate_airfoil(Re, Mach, alpha, cl_target, cl_cd_target, min_thickness, max_thickness):
    ...
    # Run pipeline (note: thickness parameters not used in new pipeline)
    pred_x, pred_y = run_pipeline_new(Re, Mach, alpha, cl_target, cd_target, cl_cd_target)
